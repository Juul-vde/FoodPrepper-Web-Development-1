...
class RecipeRepository extends BaseRepository
{
    ...
    {
        // Complex SQL query that joins recipes with categories
        // GROUP_CONCAT combines multiple categories into one row per recipe
        $sql = "SELECT r.*, 
        ...
        // Convert concatenated category strings into proper arrays
    ...
    // Helper function to convert category strings to array
    // Takes comma-separated values and creates array of category objects
    private function parseCategories($ids, $names, $icons, $colors)
    {
        // Return empty array if no categories
        if (!$ids) return [];
        
        // Split strings by comma or separator
        $idArray = explode(',', $ids);
    ...
    public function update(Recipe $recipe)
    {
        ...
        // Execute and return true if rows were changed
    ...
    {
        // Get all recipes that belong to this category
        ...
        // Parse categories for each recipe
        foreach ($recipes as &$recipe) {
            if ($recipe['category_ids']) {
                $recipe['categories'] = $this->parseCategories(
                    $recipe['category_ids'],
    ...
    // Find recipes with a specific tag
    public function findByTag($tagId)
    {
        // Get recipes that have this tag
        $sql = "SELECT DISTINCT r.*,
                GROUP_CONCAT(DISTINCT c.id ORDER BY c.display_order) as category_ids,
                GROUP_CONCAT(DISTINCT c.name ORDER BY c.display_order) as category_names,
                GROUP_CONCAT(DISTINCT c.icon ORDER BY c.display_order SEPARATOR '|||') as category_icons,
    ...
    // Get recipe with all its tags
    public function getRecipeWithTags($recipeId)
    {
        // Join recipe with tags and categories
        $sql = "SELECT r.*, 
    ...
    // Remove one ingredient from recipe
    ...
    {
        // INSERT IGNORE = don't fail if already exists
    ...
    // Remove one category from recipe
    ...
    // Remove all categories from recipe
    ...
    // Search recipes by tag ID
    public function searchByTag($tagId)
    {
        // Get all recipes with this tag, including their categories
        $sql = "SELECT DISTINCT r.*,
                GROUP_CONCAT(DISTINCT c.id ORDER BY c.display_order) as category_ids,
                GROUP_CONCAT(DISTINCT c.name ORDER BY c.display_order) as category_names,
                GROUP_CONCAT(DISTINCT c.icon ORDER BY c.display_order SEPARATOR '|||') as category_icons,
